...
<body>
    ...
    <table style="border:1px solid black; text-align:center">
        ...
        {{-- @foreach ($products as $key => $product)
            <tr>
                <td><img src="{{ asset($photos[$key]) }}" alt="" style="width:200px; height:200px"></td>
                <td>{{ $product->productName }}</td>
                <td>{{ $product->description }}</td>
                <td>{{ $product->price }}</td>
                <td>{{ $product->qty }}</td>
                <td>
                    <form action="/home/shopping-cart/{{ $product->id }}" method="post">
                        @method('DELETE')
                        @csrf
    ...
    <form method="post" action="/home/thankyou">
        @csrf
        <button type="submit" name="checkOut" value="PROFIL USER">Checkout</button>
    </form>
