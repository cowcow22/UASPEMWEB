...
<body>
    ...
    @auth
        <!-- Content -->
        <div class="border-md p-4 d-flex flex-column align-items-center rounded">
            ...
            <form action="{{ route('updateProfile') }}" method="POST" class="text-center">
                ...
                <div class="mt-4"> <!-- Removed unnecessary classes from the parent div -->
                    <ul class="list-unstyled text-start"> <!-- Apply text-start directly to ul -->
                        ...
                        <li>Socials: <input type="text" name="socialMedia"
                                value="{{ $user->socialMedia ?? 'No socials set.' }}"> </li>
                        <li>Address:
                            <textarea name="address" id="" cols="30" rows="10">{{ $user->address ?? 'Location unknown' }}</textarea>
                        </li>
                        <li>Country: <input type="text" name="country"
                                value="{{ $user->country ?? 'No country set' }}">
                        </li>
                        <li>Postal Code: <input type="text" name="postalCode"
                                value="{{ $user->postalCode ?? 'No postal code set' }}"></li>
                        <li>Phone Number: <input type="text" name="phoneNumber"
